...
async function hydrateFromCloud(){try{const r=await fetch('/api/site',{cache:'no-store'});if(!r.ok)return;const remote=await r.json(); if(remote&&remote.races&&remote.photos){ data={...data,...remote}; applyTexts(); renderStats(); renderNextRace(); renderCalendar(); renderGallery(); renderFeatured(); renderPartners(); renderNews(); renderResults(); }}catch(e){console.warn('Modo local/fallback:',e.message)}}
...
setTimeout(hydrateFromCloud,50);
function applyTexts(){
...
function bindUI(){
  ...
  qs('#contactForm').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const payload=Object.fromEntries(fd.entries());try{const r=await fetch('/api/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(!r.ok)throw new Error('Falha no envio');qs('#formNote').textContent='Mensagem enviada com sucesso.';e.currentTarget.reset()}catch(err){data.messages.push({id:Date.now().toString(),date:new Date().toISOString(),...payload,read:false});saveData();qs('#formNote').textContent='Não foi possível enviar para a Cloudflare. A mensagem ficou guardada neste navegador.'}})
}
