...
async function hydrateFromCloud(){
 ...
 cloudSyncPromise=(async()=>{
  try{
   const r=await fetch('/api/site',{cache:'no-store',headers:{Accept:'application/json'}});
   const remote=await r.json().catch(()=>({}));
   if(!r.ok) throw new Error(remote.error||remote.detail||('HTTP '+r.status));
   if(!remote||!remote.settings||!remote.texts||!Array.isArray(remote.races)||!Array.isArray(remote.photos)||!Array.isArray(remote.partners)||!Array.isArray(remote.news)||!Array.isArray(remote.results)) throw new Error('Resposta da Cloudflare incompleta.');
   data=remote;
   applyTexts();renderStats();renderNextRace();renderCalendar();renderGallery();renderFeatured();renderPartners();renderNews();renderResults();
   document.body.classList.remove('cloud-error');
   return true;
  }catch(e){
   console.error('Erro de sincronização:',e);
   document.body.classList.add('cloud-error');
   const note=document.createElement('div'); note.id='cloudSyncError'; note.textContent='Não foi possível carregar os dados da Cloudflare. Recarrega a página ou verifica o deployment.'; document.body.appendChild(note);
   return false;
  }
 })();
...
document.addEventListener('DOMContentLoaded',async()=>{
 bindUI();applyTexts();renderStats();renderNextRace();renderCalendar();renderGallery();renderFeatured();renderPartners();renderNews();renderResults();setupReveal();await hydrateFromCloud();setTimeout(()=>qs('#site-loader')?.classList.add('hide'),250);
});
function applyTexts(){
...
function bindUI(){
  ...
  qs('#contactForm').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const payload=Object.fromEntries(fd.entries());const note=qs('#formNote');note.textContent='A enviar…';try{const r=await fetch('/api/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.error||'Falha no envio');note.textContent='Mensagem enviada com sucesso.';e.currentTarget.reset()}catch(err){note.textContent='Não foi possível enviar a mensagem. Tenta novamente.';console.error(err)}})
}
