(() => {
  ...
  async function upload(file){const fd=new FormData();fd.append('file',file,file.name);return (await request('/api/media',{method:'POST',body:fd})).media}
  async function deleteMedia(id){return (await request(`/api/media?id=${encodeURIComponent(id)}`,{method:'DELETE'})).ok===true}
  window.TPCloud={checkCloud,session,login,logout,pullData,pushData,listMedia,listReviews,saveEntity,createEntity,deleteEntity,patchSettings,pullMessages,markMessageRead,clearMessages,addMessage,addReview,upload,deleteMedia,lastError:()=>lastError,getHealth:()=>health};
})();
