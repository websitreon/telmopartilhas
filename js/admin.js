...
async function wireImageUploads(){document.querySelectorAll('[data-upload-btn]').forEach(btn=>btn.onclick=async()=>{const name=btn.dataset.uploadBtn;const file=document.querySelector(`[data-image-upload="${name}"]`)?.files?.[0];const target=document.querySelector(`[data-image-target="${name}"]`);const status=document.querySelector(`[data-upload-status="${name}"]`);if(!file){status.textContent='Escolhe primeiro uma imagem.';return}btn.disabled=true;status.textContent='A carregar para Cloudflare…';try{const fd=new FormData();fd.append('file',file);const r=await fetch('/api/upload',{method:'POST',body:fd,credentials:'same-origin'});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Falha no upload');target.value=j.url;status.textContent='Imagem carregada e guardada no R2.'}catch(e){status.textContent=e.message||'Não foi possível carregar a imagem.'}finally{btn.disabled=false}})}
...
async function setup(){
  ...
  try{cloudMode=window.TPCloud?await window.TPCloud.checkCloud():false;cloudMessages=cloudMode?await window.TPCloud.pullMessages():getMessages();}catch{cloudMessages=getMessages();}
  const login=async()=>{
