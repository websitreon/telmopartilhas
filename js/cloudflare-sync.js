(() => {
  ...
  async function pushData(data) {
    ...
    try {
      ...
      cloudAvailable = cloudAvailable === null ? false : cloudAvailable;
      return { ok: false, error: e.message };
    }
  }

  async function login(password) {
    return request('/api/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) });
  }

  async function logout() {
    try { await request('/api/auth', { method: 'DELETE' }); } catch {}
  }
  ...
  async function clearMessages() {
    try { const r = await request('/api/messages', { method: 'DELETE' }); if (r.ok) localStorage.removeItem(MSG); return !!r.ok; } catch { return false; }
  }

  async function checkCloud() {
    try { const r = await request('/api/state'); cloudAvailable = !!r.ok; return cloudAvailable; } catch { cloudAvailable = false; return false; }
  }

  window.TPCloud = { pullData, pushData, login, logout, pullMessages, addMessage, clearMessages, checkCloud,
