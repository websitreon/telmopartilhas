...
async function refreshFromCloud(){
  if(!window.TPCloud)return false;
  try{
    const remote=await window.TPCloud.pullData(data);
    if(remote && JSON.stringify(remote)!==JSON.stringify(data)){data=normalizeCompetitionCategories(remote);localStorage.setItem(KEY,JSON.stringify(data));render();return true}
  }catch{}
  return false;
}
function setHtml(id,value){const node=el(id);if(node)node.innerHTML=String(value??'')}
function setText(id,value){const node=el(id);if(node)node.textContent=String(value??'')}
function splitLines(value,fallback=[]){const a=String(value??'').split('|').map(x=>x.trim()).filter(Boolean);return a.length?a:fallback}
function updateMeta(){const s=data.settings||{};const meta=document.querySelector('meta[name=description]');if(meta)meta.content=`${s.siteName||'Telmo Partilhas'} — ${s.heroSubtitle||'Rallycross, Motorsport e Photography.'}`;}
function renderSettings(){
  const s=data.settings||{};
  updateMeta();
  document.title=s.siteName?`${s.siteName} | Rallycross • Motorsport • Photography`:'Telmo Partilhas | Rallycross • Motorsport • Photography';
  setText('#heroEyebrow',s.heroEyebrow||'RALLYCROSS · MOTORSPORT · PHOTOGRAPHY');
  setText('#heroTitle',s.heroTitle||'O Rallycross através da minha lente.');
  setText('#heroSubtitle',s.heroSubtitle||'Fotografia de Rallycross, Motorsport e momentos que ficam para a história.');
  setText('#heroGalleryBtn',s.heroGalleryButton||'Ver fotografias'); if(el('#heroGalleryBtn'))el('#heroGalleryBtn').innerHTML=escText(s.heroGalleryButton||'Ver fotografias')+' <b>↗</b>';
  setText('#heroCalendarBtn',s.heroCalendarButton||'Calendário de provas'); if(el('#heroCalendarBtn'))el('#heroCalendarBtn').innerHTML=escText(s.heroCalendarButton||'Calendário de provas')+' <b>→</b>';
  const tags=splitLines(s.heroTags,['RALLYCROSS','MOTORSPORT','PHOTOGRAPHY','PORTUGAL']); if(el('#heroTags'))el('#heroTags').innerHTML=tags.map(x=>`<span>${escText(x)}</span>`).join('');
  const mq=splitLines(s.marqueeText,['RALLYCROSS','MOTORSPORT','PHOTOGRAPHY']); if(el('#marqueeTrack'))el('#marqueeTrack').innerHTML=[...mq,...mq].map((x,i)=>`<span>${escText(x)}</span><b>✦</b>`).join('');
  setText('#aboutKicker',s.aboutKicker||'01 / SOBRE'); setText('#aboutTitle',s.aboutTitle||'Sobre o meu trabalho.'); setText('#aboutText',s.aboutLead||s.about||'');
  setText('#nextKicker',s.nextKicker||'02 / PRÓXIMA PROVA'); setText('#nextLive',s.nextLive||'AO VIVO EM BREVE');
  setText('#calendarKicker',s.calendarKicker||'03 / CALENDÁRIO'); setText('#calendarTitle',s.calendarTitle||'Calendário de provas.'); setText('#calendarLead',s.calendarLead||'');
  setText('#galleryKicker',s.galleryKicker||'04 / GALERIA'); setText('#galleryTitle',s.galleryTitle||'Momentos em movimento.'); setText('#galleryLead',s.galleryLead||'');
  setText('#partnersKicker',s.partnersKicker||'05 / PARCEIROS'); setText('#partnersTitle',s.partnersTitle||'Parceiros que tornam este projeto possível.'); setText('#partnersLead',s.partnersLead||'');
  setText('#newsKicker',s.newsKicker||'06 / NOTÍCIAS'); setText('#newsTitle',s.newsTitle||'Últimas notícias.'); setText('#newsLead',s.newsLead||'');
  setText('#resultsKicker',s.resultsKicker||'07 / RESULTADOS'); setText('#resultsTitle',s.resultsTitle||'Resultados em destaque.'); setText('#resultsLead',s.resultsLead||'');
  setText('#contactsKicker',s.contactsKicker||'08 / CONTACTOS'); setText('#contactsTitle',s.contactsTitle||'Vamos falar de Motorsport.'); setText('#contactsLead',s.contactsLead||'');
  setText('#collaborationKicker',s.collaborationKicker||'COLABORAÇÕES'); setText('#collaborationTitle',s.collaborationTitle||'Queres colaborar ou ser parceiro?'); setText('#collaborationLead',s.collaborationLead||''); if(el('#collaborationButton'))el('#collaborationButton').innerHTML=escText(s.collaborationButton||'Falar sobre parcerias')+' <b>↗</b>';
  setText('#socialsKicker',s.socialsKicker||'REDES SOCIAIS'); setText('#socialsTitle',s.socialsTitle||'Segue o meu trabalho.');
  setText('#footerText',s.footer||'Rallycross · Motorsport · Photography'); setText('#footerRights',s.footerRights||'Todos os direitos reservados.');
  document.querySelectorAll('[data-nav]').forEach(a=>{const key=a.dataset.nav;const labels={home:s.navHome||'Início',about:s.navAbout||'Sobre',calendar:s.navCalendar||'Calendário',results:s.navResults||'Resultados',gallery:s.navGallery||'Galeria',partners:s.navPartners||'Parceiros',news:s.navNews||'Notícias',contacts:s.navContacts||'Contactos',galleryCta:s.navGalleryCta||'Ver galeria'};const txt=labels[key];if(txt && a.classList.contains('nav-cta'))a.innerHTML=escText(txt)+' <span>↗</span>';else if(txt)a.textContent=txt});
  const img=el('.brand img');if(img)img.alt=s.siteName||'Telmo Partilhas'; const flogo=el('.footer-logo');if(flogo)flogo.alt=s.siteName||'Telmo Partilhas';
  setText('#siteTitle',s.siteName?`${s.siteName} | Rallycross • Motorsport • Photography`:'Telmo Partilhas | Rallycross • Motorsport • Photography');
...
render();
(async()=>{await refreshFromCloud();setInterval(refreshFromCloud,10000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshFromCloud()});window.addEventListener('pageshow',refreshFromCloud)})();
