...
async function isAdmin(request,env){
  const secret=env.ADMIN_PASSWORD || 'telmo2027';
...
async function api(request,env){
  ...
  if(path==='/api/health' && request.method==='GET'){
    let d1=false, tables=false;
    if(env.DB){
      d1=true;
      try{ await ensureTables(env); tables=true; }catch(e){}
    }
    return json({ok:true,d1,tables,r2:!!env.MEDIA,adminPassword:!!env.ADMIN_PASSWORD});
  }
  if(path==='/api/auth' && request.method==='GET') return json({ok:await isAdmin(request,env)});
  if(path==='/api/auth' && request.method==='POST'){
    const adminPassword=env.ADMIN_PASSWORD || 'telmo2027';
    let body; try{body=await request.json();}catch{return json({ok:false,error:'Pedido inválido.'},400);}
    if(body.password!==adminPassword) return json({ok:false,error:'Password incorreta.'},401);
    const token=await makeSession(adminPassword);
  ...
  if(path==='/api/messages' && request.method==='PATCH'){
    ...
    if(!env.DB) return json({ok:false,error:'D1 não está ligado.'},503);
    let body; try{body=await request.json();}catch{return json({ok:false,error:'JSON inválido.'},400);}
    if(!body.id) return json({ok:false,error:'ID em falta.'},400);
    await ensureTables(env);
    await env.DB.prepare('UPDATE messages SET read=? WHERE id=?').bind(body.read?1:0,String(body.id)).run();
    return json({ok:true});
  }
